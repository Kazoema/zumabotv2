...
const instagram = 'https://instagram.com/lord.zuma45'; // Nama Instagramlu cok
const groupwa = 'comming soon'; // OFFICIAL GRUP LU 1
const youtube = 'https://youtube.com/channel/UChuGBhIabLuwGVIGuJkcJ_g'; 
...
const vcard = 'BEGIN:VCARD\n' 
            + 'VERSION:3.0\n' 
            + 'FN:Lord Kazoema\n' 
            + 'ORG: Pengembang ZumaBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=6283818003060:+62 838-1800-3060\n' 
            + 'END:VCARD' 
prefix = '#'
blocked = []          
/*********** LOAD FILE ***********/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr already scan.subscribe','white'),color('YOU','red'),color('TUBE','white'),color('OneDeath Channel','yellow'))
})
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6283818003060@s.whatsapp.net"] 
          ...
          if (!isRegistered) return reply(ind.noregis())
            ...
            break
				case 'info':
					...
					teks = `*Nama bot* : ${me.name}\n*OWNER* : *Kazoema*\n*IG* : https://instagram.com/lord.zuma45\n*AUTHOR* : Lord Zuma\n*Nomor Bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total Block Contact* : ${blocked.length}\n*The bot is active on* : ${kyun(uptime)}`
       ...
       client.sendMessage(from, 'Tuh nomer owner ku >_<, jangan dispam Ngentot atau ku block kamu',MessageType.text, { quoted: mek} )
           break    
           case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `𝗦𝗲𝗹𝗮𝗺𝗮𝘁🥳 𝗮𝗻𝗱𝗮 𝗻𝗮𝗶𝗸 𝗺𝗲𝗻𝗷𝗮𝗱𝗶 ??𝗱𝗺𝗶𝗻 𝗴𝗿𝗼𝘂𝗽 (+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
				...
				if (getLevelingLevel(sender) < ahb ) return reply(ind.reglevelahb(command, pushname, getLevelingLevel, sender, ahb))
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '𝗱𝗮?? *Jangan Lupa Follow IG Owner\n*IG:* https://instagram.com/lord.zuma45 '})
						fs.unlinkSync(ran)
					})
					break
                 case 'simi':
